...
def conv_net(image, label, filt1, filt2, bias1, bias2, theta3, bias3):
    """Returns gradient for all parameters in each iteration"""
    ...
    #  Back propagation to get gradient	using chain rule of differentiation
...
def initialise_param_lecun_normal(FILTER_SIZE, IMG_DEPTH, scale=1.0):
    if scale <= 0.:
        raise ValueError('`scale` must be a positive float. got: {}'.format(scale))
...
def momentumGradDescent(batch, LEARNING_RATE, w, l, MU, filt1, filt2, bias1, bias2, theta3, bias3, cost, acc):
    ...
    for i in range(0, batch_size):
        ...
        dfilt1_, dfilt2_, dbias1_, dbias2_, dtheta3_, dbias3_, curr_cost, acc_ = conv_net(image, label, filt1, filt2,
